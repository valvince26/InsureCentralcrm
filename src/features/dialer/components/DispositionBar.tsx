...
export default function DispositionBar() {
  ...
  return (
    <footer className="h-[100px] bg-white border-t border-outline-variant flex items-center px-8 gap-4 z-40 shadow-[0_-4px_12px_rgba(0,0,0,0.03)] flex-shrink-0">
        ...
        {dispositions.map((disp) => {
          ...
          return (
            <button
              ...
              className={`h-full px-6 flex items-center justify-center gap-2 rounded-xl font-label-md transition-all cursor-pointer ${baseClass} ${activeStyle}`}
            >
      ...
      <button className="h-14 px-8 bg-secondary text-white rounded-xl font-title-md flex items-center gap-3 shadow-lg hover:shadow-xl transition-all cursor-pointer flex-shrink-0">
        Submit & Next <span className="material-symbols-outlined">chevron_right</span>
      </button>
    </footer>
  );
}
